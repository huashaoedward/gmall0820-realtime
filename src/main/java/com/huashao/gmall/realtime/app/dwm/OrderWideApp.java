...
import com.huashao.gmall.realtime.bean.OrderDetail;
import com.huashao.gmall.realtime.bean.OrderInfo;
 ...
 * Author: huashao
 * Date: 2021/8/3
 * Desc: 合并订单宽表。结订单相关的事实和维度信息进行关联，形成宽表。
 * 测试需要启动服务：
...
public class OrderWideApp {
    public static void main(String[] args) throws Exception {
        ...
        //3.1 转换订单信息order_info数据结构， jsonString -->OrderInfo
        SingleOutputStreamOperator<OrderInfo> orderInfoDS = orderInfoJsonStrDS.map(
            new RichMapFunction<String, OrderInfo>() {
                ...
                public void open(Configuration parameters) throws Exception {
                    //把"yyyy-MM-ddHH:mm:ss"格式的字符串，转为date对象，
                    sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                }

                @Override
                public OrderInfo map(String jsonStr) throws Exception {
                    //！！！使用fastJson，把json类型的字符串，转换为OrderInfo的bean对象
                    OrderInfo orderInfo = JSON.parseObject(jsonStr, OrderInfo.class);
                    /*
                    sdf.parse把"yyyy-MM-ddHH:mm:ss"格式的字符串，转为date对象，
                    getTime再拿到Date对象中的String格式的时间戳；
                    再传给Bean对象的setCreate_ts()
                     */
        ...
        //3.2 转换订单明细order_detail数据结构， jsonStr转为orderDetail
        ...
        SingleOutputStreamOperator<OrderInfo> orderInfoWithTsDS = orderInfoDS.assignTimestampsAndWatermarks(
            WatermarkStrategy
                    //指定最大乱序度是3秒，前面要加泛型，表示流里放的数据是OrderInfo对象
                .<OrderInfo>forBoundedOutOfOrderness(Duration.ofSeconds(3))
                    //指定时间戳分配器，并重写里面提取时间戳的方法extractTimestamp
                .withTimestampAssigner(
                        new SerializableTimestampAssigner<OrderInfo>() {
                    @Override
                    public long extractTimestamp(OrderInfo orderInfo, long recordTimestamp) {
                        //以订单信息orderInfo的Create_ts字段的时间为准
        ...
        //lambda表达式，按OrderInfo里的getId所返回的订单id来分组
        ...
        SingleOutputStreamOperator<OrderWide> orderWideDS = orderInfoKeyedDS
            ...
            .process(
                    //传入ProcessJoinFunction对象，泛型<IN1, IN2, OUT>
                new ProcessJoinFunction<OrderInfo, OrderDetail, OrderWide>() {
                    @Override
                    public void processElement(OrderInfo orderInfo, OrderDetail orderDetail, Context ctx, Collector<OrderWide> out) throws Exception {
                        /*
                        使用Collector发送数据，是一个OrderWide的对象,里面放的是(orderInfo,orderDetail)两个对象；
                        OrderWide是订单宽表的实体类，也是一个bean，对应订单宽表，它的构造参数是orderInfo, orderDetail
                        */
        ...
        SingleOutputStreamOperator<OrderWide> orderWideWithUserDS = AsyncDataStream.unorderedWait(
            orderWideDS,
            /*
            因为DimAsyncFunction是抽象类，所以创建对象一定要实现里面的方法，要传入要查询的维度表。
            给出具体的getKey和join的方案
            注意这里要传入泛型OrderWide
            */
            new DimAsyncFunction<OrderWide>("DIM_USER_INFO") {
                ...
                /*把用户的年龄和性别关联上
                //注意！！这里的dimInfoJsonObj，是根据传入的要查询的维度表，DimUtil
                .getDimInfo(tableName,key)查询出来的信息
                看DimAsyncFunction，
                */
                @Override
                //重写父类的join方法
                public void join(OrderWide orderWide, JSONObject dimInfoJsonObj) throws Exception {
                    //获取用户生日
                    //这里的dimInfoJsonObj，是根据传入的要查询的维度表，DimUtil.getDimInfo(tableName,key)查询出来的信息
        ...
        SingleOutputStreamOperator<OrderWide> orderWideWithProvinceDS = AsyncDataStream.unorderedWait(
            orderWideWithUserDS,
                //泛型是OrderWide，要关联的维度表是DIM_BASE_PROVINCE
        ...
        SingleOutputStreamOperator<OrderWide> orderWideWithSkuDS = AsyncDataStream.unorderedWait(
            ...
            //泛型是OrderWide，要关联的维度表是DIM_SKU_INFO
        ...
        SingleOutputStreamOperator<OrderWide> orderWideWithSpuDS = AsyncDataStream.unorderedWait(
            orderWideWithSkuDS,
                //泛型是OrderWide，要关联的维度表是DIM_SPU_INFO
            new DimAsyncFunction<OrderWide>("DIM_SPU_INFO") {
                ...
                @Override
                //dimInfoJsonObj是根据传入的要查询的维度表，DimUtil.getDimInfo(tableName,key)查询出来的信息
                public void join(OrderWide orderWide, JSONObject dimInfoJsonObj) throws Exception {
        ...
        SingleOutputStreamOperator<OrderWide> orderWideWithCategory3DS = AsyncDataStream.unorderedWait(
            orderWideWithSpuDS,
                //泛型是OrderWide，要关联的维度表是DIM_BASE_CATEGORY3
